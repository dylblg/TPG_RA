...
public class MainActivity extends ListActivity{
	...
	protected void onCreate( Bundle savedInstanceState ) {
		...
		final List<SampleMeta> activitiesToLaunch = getActivitiesToLaunch(/*position*/5);
	...
	protected final String[] getListLabels() {
		...
		final String[] labels = new String[/*samples.keySet().size()*/1];
		for (int i = 0; i<(labels.length); i++) {
			labels[i] = samples.get(5).get(5).categoryId + ". " + samples.get(5).get(5).categoryName.replace("$", " ");
		}
		return labels;
	}
